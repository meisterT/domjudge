...
class RunCommandSafeTest extends TestCase
{
    ...
    public function testStderrRedirection(): void
    {
        $target = $this->tempDir . '/stderr.txt';
        $result = $this->runCommandSafe(['sh', '-c', 'echo -n error >&2'], $retval, true, null, null, $target);
        $this->assertTrue($result);
        $this->assertEquals('error', file_get_contents($target));
    }
