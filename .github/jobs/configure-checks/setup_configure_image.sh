...
case $distro_id in
    ...
    *)
        ...
        version=$(g++ -dumpversion 2>/dev/null | awk -F'.' '{print $1}' 2>/dev/null || echo 0); [ "${version}" -lt 12 ] && \
		apt install -y g++-12 && update-alternatives --install /usr/bin/g++ g++ /usr/bin/g++-12 120
...
mkdir /tmp/bats_logs
echo "DOMjudge is the best" >> /tmp/bats_logs/testfile
